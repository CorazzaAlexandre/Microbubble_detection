...
%
% DATE 2023.04.25 - VERSION 1.2
% Modified by Alexandre Corazza: Decision theory based detection method added. Only parts annoted with the "added/modified by A. Corazza" comment have been changed
% Some parameters have been added:
% set ULM.parameters.DetectMethod to 'Intensity' for the intensity-based detection method, 'crosscor' for the normalized cross-correlation (NCC) or 'NP' for the Neyman Pearson
% for the NCC:
% set ULM.parameters.MB_image: the NCC kernel
% set ULM.parameters.crosscor_threshold: the NCC threshold
% for the NP:
% set ULM.parameters.NP_alpha0: the false alarm rate.
% Details of the code in the article by Corazza, Muleki-Seya, Basarab, Nicolas,
% Microbubble identification based on decision theory for Ultrasound Localization Microscopy, OJ-UFFC 2023
...
MatIn_origin = MatIn; %Matrix on which the localisation algorithm will be applied (%added by A. Corazza)
...
MatInReduced_formetric = zeros(height,width,numberOfFrames,typename);

if isequal(ULM.parameters.DetectMethod, 'NP') %(added by A. Corazza)
    %reshape into Casorati matrix
    [Nz, Nx, Nt] = size(MatIn_origin);
    Casorati = abs(reshape(MatIn_origin, [Nx*Nz, Nt]));
    
    %compute noise statistic moments
    moy = median(Casorati, 2);
    stdev =  (1/(erfinv(0.5)*sqrt(2))) * mad(Casorati, 1, 2);

    %compute the detection maps
    thresh = sqrt(2) * stdev .* erfinv(1-2*ULM.parameters.NP_alpha0) + moy;
    detection_map_Casorati = (Casorati - thresh) .* (Casorati - thresh >= 0); %M - col vector = all M col - col vector
    detection_map = reshape(detection_map_Casorati, [Nz, Nx, Nt]);
    MatIn = abs(detection_map);
    MatIn(isnan(MatIn)) = 0; %pour 3pDAS par exemple
    
    
elseif isequal(ULM.parameters.DetectMethod, 'crosscor') %(added by A. Corazza)
    [Nz, Nx, Nt] = size(MatIn_origin);
    detection_map = zeros(Nz, Nx, Nt);
    
    for k = 1:size(MatIn_origin, 3)
        img = abs(MatIn_origin(:,:,k));
        cross_corr = normxcorr2(ULM.parameters.MB_image, img);
        cross_corr = cross_corr.*(cross_corr>=ULM.parameters.crosscor_threshold);
        %size of template must be odd
        sx_template = (size(ULM.parameters.MB_image,1)-1)/2;
        sz_template = (size(ULM.parameters.MB_image,2)-1)/2;
        detection_map(:, :, k) = cross_corr(sx_template+1:end-sx_template, sz_template+1:end-sz_template); %crop the image to make the dimensions match
    end
    MatIn = abs(detection_map);
end
...
MatInReduced_formetric(1+round(fwhmz/2)+1:height-round(fwhmz/2)-1,1+round(fwhmx/2)+1:width-round(fwhmx/2)-1,:) = MatIn_origin(1+round(fwhmz/2)+1:height-round(fwhmz/2)-1, 1+round(fwhmx/2)+1:width-round(fwhmx/2)-1,:);
...
for iscat=1:size(index_mask_z,1)
    ...
    IntensityRoi = MatIn_origin(index_mask_z(iscat)+vectfwhmz,index_mask_x(iscat)+vectfwhmx,index_numberOfFrames(iscat)); %modified by A. Corazza
...
MatTracking(:,1) = MatInReduced_formetric(ind);       % Initial intensity of the microbubble
